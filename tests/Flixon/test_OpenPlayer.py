...
@allure.description("""The test opens video player from the Cinema page and verifies that the player
contains all the controls and those are up and running, recorded stream""")
def test_player_from_cinema_initial_state_verification():
    with allure.step("Sign In as an existing user"):
        SigninPage().login_as_user()
    # with allure.step("Click Yes on the Personal Video Recording popup"):
    #     time.sleep(10)
...
def test_player_from_recordings_initial_state_verification():
    ...
    with allure.step("Open the banner for the 1st event on Recording page"):
...
def test_player_from_search_initial_state_verification():
    ...
    with allure.step("Set 'Cold Squad' as the Search criterion"):
        SearchPage().search_input_field.set_value("cold")
        time.sleep(1)
    with allure.step("Click on the first event from those found"):
        s(by.xpath("(//BUTTON[@class='item'])[1]")).click()
        time.sleep(2)
